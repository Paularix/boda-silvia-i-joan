...
export const MobileNav = ({ activeSection, onNavigate }: { activeSection: string; onNavigate: (s: string) => void }) => {
  ...
  // --- ESTA ES LA FUNCIÓN QUE TIENES QUE AÑADIR ---
  const handleNavClick = (id: string) => {
    setOpen(false); // Primero cerramos el menú lateral
    // Esperamos un pelín a que el menú empiece a cerrarse para no bloquear el scroll
    setTimeout(() => {
      onNavigate(id);
    }, 150);
  };
  ...
  const navBtn = (s: typeof sections[0]) => (
    <Button 
      key={s.id} 
      onClick={() => handleNavClick(s.id)} // Usamos la nueva función
      startIcon={s.icon}
      sx={{ 
        color: activeSection === s.id ? C.roseDark : C.slateLight, 
        fontWeight: activeSection === s.id ? 700 : 400, 
        fontSize: 11, 
        letterSpacing: '0.12em', 
        textTransform: 'uppercase', 
        borderBottom: activeSection === s.id ? `2px solid ${C.rose}` : '2px solid transparent', 
        borderRadius: 0, 
        px: 2, 
        py: 1.5, 
        '&:hover': { bgcolor: 'transparent', color: C.roseDark }, 
  ...
  return (
    <>
      <IconButton 
        ...
        sx={{ position: 'fixed', bottom: 24, right: 24, zIndex: 1000, bgcolor: C.slate, color: C.cream, width: 52, height: 52, boxShadow: '0 4px 16px rgba(61,53,48,0.25)', '&:hover': { bgcolor: C.slateLight } }}
      >
        <MenuIcon />
      </IconButton>
            ...
            <Box sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                {sections.map(s => (
                  <Button 
                    key={s.id} 
                    onClick={() => handleNavClick(s.id)} // Usamos la nueva función aquí también
                    startIcon={s.icon} 
                    fullWidth
                    sx={{ justifyContent: 'flex-start', color: activeSection === s.id ? C.roseDark : C.slateLight, fontWeight: activeSection === s.id ? 700 : 400, fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', py: 1.5, '&:hover': { bgcolor: C.cream } }}
                  >
                    {s.label}
                  </Button>
                ))}
